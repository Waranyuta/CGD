    ...
    with torch.no_grad():
        ...
        for key in eval_dict.keys():
            ...
            for inputs, labels in tqdm(
                eval_dict[key]["data_loader"], desc="processing {} data".format(key)
            ):
                inputs, labels = inputs.cuda(), labels.cuda()
                features, classes = net(inputs)
        ...
        if data_name == "isc":
            ...
            acc_list = recall(
                eval_dict["test"]["features"], test_data_set.labels, recall_ids
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0.1,
        type=float,
        help="smoothing value for label smoothing",
    )
    parser.add_argument(
        "--temperature",
        default=0.5,
    ...
    parser.add_argument(
        "--margin", default=0.1, type=float, help="margin of m for triplet loss"
    )
    parser.add_argument(
        "--recalls", default="1,2,4,8", type=str, help="selected recall"
    )
    ...
    # args parse
    data_path, data_name, crop_type, backbone_type = (
        opt.data_path,
        opt.data_name,
        opt.crop_type,
        opt.backbone_type,
    )
    gd_config, feature_dim, smoothing, temperature = (
        opt.gd_config,
        opt.feature_dim,
        opt.smoothing,
        opt.temperature,
    )
    margin, recalls, batch_size = (
        opt.margin,
    ...
    train_data_loader = DataLoader(
        train_data_set, batch_sampler=train_sample, num_workers=8
    )
    test_data_set = ImageReader(
        data_path, data_name, "query" if data_name == "isc" else "test", crop_type
    )
    test_data_loader = DataLoader(
        test_data_set, batch_size, shuffle=False, num_workers=8
    )
    eval_dict = {"test": {"data_loader": test_data_loader}}
    if data_name == "isc":
        gallery_data_set = ImageReader(data_path, data_name, "gallery", crop_type)
        gallery_data_loader = DataLoader(
            gallery_data_set, batch_size, shuffle=False, num_workers=8
        )
    ...
    lr_scheduler = MultiStepLR(
        optimizer, milestones=[int(0.6 * num_epochs), int(0.8 * num_epochs)], gamma=0.1
    )
