...
class ImageReader(Dataset):
    def __init__(self, data_path, data_name, data_type, crop_type):
        if crop_type == "cropped" and data_name not in ["car", "cub", "osld"]:
            raise NotImplementedError("cropped data only works for car or cub dataset")

        data_dict = torch.load(
            "{}/{}/{}_data_dicts.pth".format(data_path, data_name, crop_type)
        )[data_type]
...
def recall(
    feature_vectors, feature_labels, rank, gallery_vectors=None, gallery_labels=None
):
    ...
    dist_matrix = torch.cdist(
        feature_vectors.unsqueeze(0), gallery_vectors.unsqueeze(0)
    ).squeeze(0)
    ...
    for r in rank:
        correct = (
            (gallery_labels[idx[:, 0:r]] == feature_labels.unsqueeze(dim=-1))
            .any(dim=-1)
            .float()
        )
        ...
        for _ in range(self.__len__()):
            ...
            while inds.shape[0] < self.batch_size:
                ...
                sample_label_ids = np.argwhere(
                    np.in1d(self.labels, sample_label)
                ).reshape(-1)
