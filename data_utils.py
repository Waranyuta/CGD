...
def process_osld_data(data_path, data_type, how):
    ...
    for dataset_mode in ["train", "val"]:
        images[dataset_mode] = {}
        anno_fn = f"osld-{dataset_mode}.json"
        with open(os.path.join(ANNO_DIR, anno_fn)) as f:
            data = json.load(f)

        for img_fn in tqdm(
            data, desc="process {} data for osld dataset".format(data_type)
        ):

            # for img_no, img_fn in enumerate(data, 1):
            # for bbox_no, anno in enumerate(data[img_fn], 1):
            for anno in data[img_fn]:

                print(bbox_no)
                bbox, label = anno
                if how == "logo":
                    label = (
                        label.split(".")[0] if not label.startswith("__") else "unknown"
                    )
                elif how == "brand":
                    label = (
                        label.split("-")[0] if not label.startswith("__") else "unknown"
                ...
                if data_type == "cropped":
                    img = Image.open(os.path.join(data_path, "product-images", img_fn))
                    left, top, right, bottom = bbox
                    img = img.crop((left, top, right, bottom))

                save_name = os.path.join(
                    data_path, data_type, f"{label}_{str(bbox_no).zfill(3)}.jpg"
                )
                img.save(save_name)

                if label in images[dataset_mode]:
                    images[dataset_mode][label].append(save_name)
                else:
                    images[dataset_mode][label] = [save_name]
                bbox_no += 1

    torch.save(
        {"train": images["train"], "test": images["val"]},
        "{}/{}_data_dicts.pth".format(data_path, data_type),
    )
...
def process_car_data(data_path, data_type):
    ...
    for img in tqdm(
        annotations, desc="process {} data for car dataset".format(data_type)
    ):
        ...
        if data_type == "uncropped":
            ...
            img = (
                Image.open("{}/{}".format(data_path, img_name))
                .convert("RGB")
                .crop((x1, y1, x2, y2))
            )
...
def process_cub_data(data_path, data_type):
    ...
    for img_id, img_name in tqdm(
        ...
        if data_type == "uncropped":
            ...
            x1, y1 = int(float(bounding_boxes[img_id][0])), int(
                float(bounding_boxes[img_id][1])
            )
            x2, y2 = x1 + int(float(bounding_boxes[img_id][2])), y1 + int(
                float(bounding_boxes[img_id][3])
            )
            img = (
                Image.open("{}/images/{}".format(data_path, img_name))
                .convert("RGB")
                .crop((x1, y1, x2, y2))
            )
        save_name = "{}/{}/{}".format(data_path, data_type, os.path.basename(img_name))
...
def process_sop_data(data_path):
    ...
    for data_type, image_list in data_tuple.items():
        for index, line in enumerate(
            open("{}/Ebay_{}.txt".format(data_path, data_type), "r", encoding="utf-8")
        ):
            if index != 0:
                _, label, _, img_name = line.split()
                img = Image.open("{}/{}".format(data_path, img_name)).convert("RGB")
                save_name = "{}/uncropped/{}".format(
                    data_path, os.path.basename(img_name)
                )
                img.save(save_name)
...
def process_isc_data(data_path):
    ...
    for index, line in enumerate(
        open("{}/Eval/list_eval_partition.txt".format(data_path), "r", encoding="utf-8")
    ):
        if index > 1:
            img_name, label, status = line.split()
            img = Image.open("{}/Img/{}".format(data_path, img_name)).convert("RGB")
            save_name = "{}/uncropped/{}_{}".format(
                data_path, img_name.split("/")[-2], os.path.basename(img_name)
            )
            img.save(save_name)
            if status == "train":
    ...
    torch.save(
        {"train": train_images, "query": query_images, "gallery": gallery_images},
        "{}/uncropped_data_dicts.pth".format(data_path),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", default="/home/data", type=str, help="datasets path"
    )

    opt = parser.parse_args()

    process_osld_data("{}/osld".format(opt.data_path), data_type="cropped", how="logo")
